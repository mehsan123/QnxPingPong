...
def arguments():
    ...
    parser.add_argument("TCF_DIR",help="TCF directory, location of LDRA unit test case files")
...
def main():
    try:
        ...
        if not os.path.isdir(args.TCF_DIR):
            raise argparse.ArgumentTypeError("TCF_DIR: {} is not a valid directory".format(args.TCF_DIR))
        ...
        for file in os.listdir(args.BUILD_DIR):
            if any(check in file for check in build_file_lookup):
                ...
                if static_result:
                    ...
                    for test in os.listdir(args.TCF_DIR):
                        if any(ext in test for ext in unit_file_lookup):
                            tcf_file = os.path.join(args.TCF_DIR,test)
                            logger.info("Execute Unit Test {} : {}".format(file,test))
                            #unit_result = tbrun.run(args.setname, "-tcf={}".format(tcf_file), "-tcf_mode=overwrite", "-regress","-quit")
                            unit_result = tbrun.run(build_file, "-tcf={}".format(tcf_file), "-tcf_mode=overwrite", "-regress","-quit")

                            if unit_result:
                                errors_detected += log_result(unit_result)
                            else:
                                logger.error("Issue with unit test {} - {}".format(build_file,tcf_file))
                                errors_detected+=1
                else:
